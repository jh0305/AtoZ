...
<script id="contract-template" type="text/x-handlebars-template">
	<table class="table card-table table-vcenter text-nowrap datatable">
		<thead>
			<tr>
				<th style="width: 6%;text-align: center;">계약번호	</th>
		...
		<tbody>
			...
			{{#each crList}}
			<tr class="whtr">
				<td style="text-align: center;"><a class="page-link" href="javascript:;"
					onclick="openDial($('#contract'),700,600,this);" tabindex="-1" aria-disabled="true">{{cc_no}}</a> 
				<input type="hidden" value="{{cl_code }}"></td>
				<td><a href="javascript:;" onclick="openDial($('#company'),700,600,this);">{{cl_name}}</a></td>
				<td><span class="text-muted" name="addr">{{cr_ymd}}</span></td>
				<td><span class="text-muted" name="typenames">{{exp_ymd}}
				</span></td>
				<td><span class="text-muted">{{cr_area}}</span></td>
				<td><span class="text-muted">{{cm_name}}</span></td>
...
<script id="request-template" type="text/x-handlebars-template">
	<table class="table card-table table-vcenter text-nowrap datatable">
		<thead>
			<tr>
				<th style="width: 6%;text-align: center;">계약요청번호	</th>
				<th style="width: 200px;">기업명</th>
				<th>계약요청총면적(㎡)</th>
				<th>계약상태</th>
				<th>희망계약기간</th>
		...
		<tbody>
			...
			<tr class="whtr">
				<td style="text-align: center;"><a class="page-link" href="javascript:;"
					onclick="openDial($('#contract'),700,600,this);" tabindex="-1" aria-disabled="true">{{cc_no}}</a> 
				<input type="hidden" value="{{co_code }}"></td>
				<td><a href="javascript:;" onclick="openDial($('#company'),700,600,this);">{{cl_name}}</a></td>

...
<style>
	...
	.clickableTr:hover{
						...
						<div class="col-12" style="padding: 0px 0px;">
							<div class="row" style="margin: 0px 0px 5px 0px;">
								<div style="display:inline-block; height: 36px; padding: 0px 10px 0px 0px;" class="col-11">
									<div class="col-12">
										<input id="inputData" type="text" class="form-control" placeholder="코드 및 이름을 입력하세요" style="border-radius: 3px;">
									</div>
								</div>
			...
			<div class="row">
				<div class="col-12 table-responsive" style="padding: 5px; margin: 0px;">
					<table class="table card-table table-vcenter text-nowrap datatable" border=1>
						<thead>
							<tr>
								<th class="text-center">물류센터코드</th>
								<th class="text-center">물류센터이름</th>
							</tr>
						</thead>
		...
		function goSearch(){
	    	...
	    	$.ajax({
	    		url : '<%=request.getContextPath()%>/CO/payment/getContractWH',
				type : 'post',
				data : {
...
//			search_wh_name 
			...
			// 검색 조건으로 페이지네이션 시작.
			if($(search_wh_code).attr("name") == "request_wh_code"){
				search_paymentRequestList_go();
			}else{
				search_paymentResultList_go();
			}

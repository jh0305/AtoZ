...
public class ServiceUseServiceImpl implements ServiceUseService {
	...
	public void registWHClient(ClientVO client) throws SQLException {
		...
		DongFloorVO dongFloor = new DongFloorVO();
		for(int i=0; i<client.getDong_nameList().length; i++) {
			dongFloor.setWh_code("WH"+Integer.toString(cl_no));
			dongFloor.setDong_name(client.getDong_nameList()[i]+"동");
			dongFloor.setFloor_name(client.getFloor_nameList()[i]+"층");
			dongFloor.setDf_width(client.getDf_widthList()[i]);
			dongFloor.setDf_dsc(client.getDf_dscList()[i]);
			dongFloor.setDf_lngth(client.getDf_lngthList()[i]);
			dongFloor.setDf_area(client.getDf_areaList()[i]);
			serviceUseDAO.insertDongFloor(dongFloor);
		}
	...
	public void registWHCOClient(ClientVO client) throws SQLException {
		...
		DongFloorVO dongFloor = new DongFloorVO();
		for(int i=0; i<client.getDong_nameList().length; i++) {
			dongFloor.setWh_code("WHCO"+Integer.toString(cl_no));
			dongFloor.setDong_name(client.getDong_nameList()[i]+"동");
			dongFloor.setFloor_name(client.getFloor_nameList()[i]+"층");
			dongFloor.setDf_width(client.getDf_widthList()[i]);
			dongFloor.setDf_dsc(client.getDf_dscList()[i]);

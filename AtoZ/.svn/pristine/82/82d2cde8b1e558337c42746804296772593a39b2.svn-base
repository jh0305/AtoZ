...
<div style="padding: 0px 15px;">
	...
	<div class="row">
		<div class="col-12">
			<div class="card" style="margin: 0px; height: 869px;">
				<div class="card-header">
					<select class="form-select" id="searchType" style="display: inline-block; width: 15%; display: inline;">
						...
						<option value="cl_id" ${searchMap.get('searchType') eq 'id' ? 'selected' : '' }>회원 아이디</option>
					...
					</select> <input type="text" class="form-control" id="keyword" value="${searchMap.get('keyword') }" placeholder="Search for…" style="width: 30%; display: inline;">
...
function getCompanyDetail(cl_code) {
	$.ajax({
		url : '<%=request.getContextPath()%>/SY/member/companyDetail?cl_code='+cl_code,
		type:'get',

...
public class ConstructServiceImpl implements ConstructService {
	...
	public void registZone(ZoneVO zone) throws SQLException {
		if (zone.getCm_code() == null) { //랙
			if (zone.getTrg_code().equals("none")) { //적재기준 : 없음
				constructDAO.insertRackInZone2(zone); 
			} else {
				constructDAO.insertRackInZone(zone);
			}
		} else { //구역
			constructDAO.insertZone(zone);
		}
	}
	@Override
	public void registUpZone(ZoneVO zone) throws SQLException {
		if (zone.getTrg_code().equals("none")) { //적재기준 : 없음
			constructDAO.insertRackInZoneUp2(zone); 
		} else {
			constructDAO.insertRackInZoneUp(zone);
		}
	}
	...
	@Override
	public void modifyTurninngZone(ZoneVO zone) throws SQLException {
		constructDAO.updateTurninngZone(zone);
	}
	@Override

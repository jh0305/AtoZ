...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
	...
	<style>
		...
		<div class="col-12" style="padding:0px 15px;">
			...
			<div class="row">
				<div class="col-6" style="padding-right:5px;">
					<div class="card col-12" style=" height: 868px; ">
					<div class="card-header">
						공통코드 유형
					</div>
					<div class="card-body">
						<div class="table-responsive" style="max-height:650px;height:650px;">
					...
					<!-- /card-body -->
					<div class="card-footer" style="background : white; text-align: right;">
						<input type="button" class="btn btn-light" onclick="openDial($('#dialog1'), 450, 320);" value="신규">
				...
				<div class="col-6" style="padding-left:5px;">
					...
					<div class="card-body">
						...
						<div class="table-responsive"  style="max-height:620px;height:620px;">
					...
					<!-- /card-body -->
					<div class="card-footer" style="background : white; text-align: right;">
						<input type="button" id="dialbtn" onclick="openDial($('#dialog2'),450,370);" class="btn btn-light" value="신규">
		...
		<div id="dialog1" class="dialogDiv" title="공통코드 유형 등록">
			<div class="container-xl" style="width:400px; height:200px;">
		    	<!-- 콘텐츠 헤더 -->
				<div class="page-header d-print-none">
					<div class="row align-items-center">	
						<div class="col">
	...
	<script>
		...
		function setType(type_code, type_name){
			...
			$('#codeAllCheck').prop("checked",false);
		}
		...
		function codeModifyForm(){
			...
			if(cnt > 0){
				openDial($('#dialog4'),550,620,true,function(){$('#modifyCodeAllCheck').prop("checked",false);});
		...
		function typeModifyForm(){
			...
			if(cnt > 0){
				openDial($('#dialog5'),500,620,true,function(){$('#modifyTypeAllCheck').prop("checked",false);});
		...
		function registCode(){
			...
			$.ajax({
				...
				success : function(){
					setType($('#typeCode').text(), $('#typeName').text());
					$('#codeAllCheck').prop("checked",false);
					closeDial($('#dialog2'));
				},
				error : function(e){
			...
			if(cnt > 0){
				for(var i = 0; i < str.length-1; i++){
					for(var j = i+1; j < str.length ; j++){
						if(str[i].type_name == str[j].type_name){
							alert("중복된 코드명이 존재합니다.");
							return;
						}
					}	
				}
		...
		function modifyCode(){
			...
			if(cnt > 0){
				for(var i = 0; i < str.length-1; i++){
					for(var j = i+1; j < str.length ; j++){
						if(str[i].cm_name == str[j].cm_name){
							alert("중복된 코드명이 존재합니다.");
							return;
						}
					}	
				...
				$.ajax({
					...
					success : function(){
						setType($('#typeCode').text(), $('#typeName').text());
						$('#codeAllCheck').prop("checked",false);
						$('#modifyCodeAllCheck').prop("checked",false);
						closeDial($('#dialog4'));
					},
					error : function(e){
						if(e.status == 400){
